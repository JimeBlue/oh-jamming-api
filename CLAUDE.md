...
CORS is an explicit origin allowlist built from `CLIENT_URL` (**comma-separated** — the client can be deployed in more than one place at once) plus `http://localhost:3000`, with `credentials: true`. `VERCEL_PREVIEW_PREFIX` additionally admits that Vercel project's preview deployments by pattern, because each preview gets its own hostname and cannot be listed in advance; it is a prefix rather than `*.vercel.app` because this API sends credentials, and a wildcard there would be every app anyone has ever deployed to Vercel. A wildcard origin is incompatible with credentials by spec, so the allowlist must stay explicit for the auth cookies to work. `exposedHeaders: ['WWW-Authenticate']` is also required — without it the browser hides that header from client JS, and the client can't tell an expired access token from a dead session.
